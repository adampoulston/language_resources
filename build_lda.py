...
def train_lda(source, passes, topics, workers, out_dir):
    ...
    print("Build dictionary...", file = sys.stdout)
    dictionary = corpora.Dictionary(GZIPTweetStream([source]))
    dictionary.save(out_dir + '/lda.dict')
    corpus = (dictionary.doc2bow(tweet) for tweet in GZIPTweetStream([source]))
...
if __name__ == "__main__":
    ...
    train_lda(source = args.source, passes = args.passes[0], topics = args.topics[0], workers = args.workers[0], out_dir = args.outdir)
